...
'type _OBJECT_TYPE_FLAGS
'	CaseInsensitive:1         as byte
'	UnnamedObjectsOnly:1      as byte
'	UseDefaultObject:1        as byte
'	SecurityRequired:1        as byte
'	MaintainHandleCount:1     as byte
'	MaintainTypeList:1        as byte
'	SupportsObjectCallbacks:1 as byte
'	CacheAligned:1            as byte
'end type
'type OBJECT_TYPE_FLAGS  as _OBJECT_TYPE_FLAGS
'type POBJECT_TYPE_FLAGS as _OBJECT_TYPE_FLAGS ptr
  
'TYPE _OBJECT_TYPE_INITIALIZER
'	wLength                   AS USHORT
'	ObjectTypeFlags           AS OBJECT_TYPE_FLAGS
'	ObjcetTypeCode            AS ULONG
'	InvalidAttributes         AS ULONG
'   GenericMapping            AS GENERIC_MAPPING
'	ValidAccessMask           AS ULONG
'	RetainAccess              AS ULONG
'	PoolType                  AS ULONG
'	DefaultPagedPoolCharge    AS ULONG
'	DefaultNonPagedPoolCharge AS ULONG
'   DumpProcedure             AS PVOID
'	OpenProcedure             AS PVOID
'	CloseProcedure            AS PVOID
'	DeleteProcedure           AS PVOID
'	ParseProcedure            AS PVOID
'	SecurityProcedure         AS PVOID
'	QueryNameProcedure        AS PVOID
'	OkayToCloseProcedure      AS PVOID
'end type
'type OBJECT_TYPE_INITIALIZER  as _OBJECT_TYPE_INITIALIZER
'type POBJECT_TYPE_INITIALIZER as _OBJECT_TYPE_INITIALIZER ptr

'TYPE _OBJECT_TYPE
'   TypeList                 AS LIST_ENTRY	
'   Name                     AS UNICODE_STRING				
'	DefaultObject            AS ULONGLONG
'	Index                    AS ULONG
'	TotalNumberOfObjects     AS ULONG
'	TotalNumberOfHandles     AS ULONG
'	HighWaterNumberOfObjects AS ULONG
'	HighWaterNumberOfHandles AS ULONG
'	TypeInfo                 AS OBJECT_TYPE_INITIALIZER
'	TypeLock                 AS ULONGLONG
'	Key                      AS ULONG						
'	CallbackList             AS LIST_ENTRY					
'END TYPE
'type OBJECT_TYPE  as _OBJECT_TYPE
...
type IRP__Tail__Overlay
   ...
   UNION
      TYPE
         ...
         UNION
            CurrentStackLocation  as IO_STACK_LOCATION PTR
            PacketType            as ULONG
         END UNION
      END TYPE
   END UNION
...
type _DEVICE_OBJECT
	...
	DriverObject as _DEVICE_OBJECT PTR
	NextDevice as _DEVICE_OBJECT PTR
	AttachedDevice as _DEVICE_OBJECT PTR
	CurrentIrp as PIRP
	Timer as PIO_TIMER
	Flags as ULONG
	Characteristics as ULONG
	Vpb as PVPB '__volatile
...
type _FILE_OBJECT
	...
	RelatedFileObject as _FILE_OBJECT PTR
	...
	CurrentByteOffset as LARGE_INTEGER
	Waiters as ULONG  '__volatile
	Busy as ULONG     '__volatile
	LastLock as PVOID
	Lock as KEVENT
	Event as KEVENT
	CompletionContext as PIO_COMPLETION_CONTEXT '__volatile
	IrpListLock as KSPIN_LOCK
	IrpList as LIST_ENTRY
	FileObjectExtension AS PVOID '__volatile
end type
...
TYPE SECTION_INHERIT AS _SECTION_INHERIT
